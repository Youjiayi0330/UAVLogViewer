...
self.addEventListener('message', function (event) {
  ...
  if (action === 'parse') {
    ...
    const data = event.data.file;
    parser.processData(data);
  } else if (action === 'loadType') {
    if (!parser) {
      console.warn('Parser is not initialized before loadType');
      return;
    }
    if (event.data.type) {
      parser.loadType(event.data.type.split('[')[0]);
    } else {
      console.warn('event.data.type is missing');
    }
  } else if (action === 'trimFile') {
    if (!parser) {
      console.warn('Parser is not initialized before trimFile');
      return;
    }
    parser.trimFile(event.data.time);
  }
